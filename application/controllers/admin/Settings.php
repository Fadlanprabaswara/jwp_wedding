...
class Settings extends CI_Controller {
        ...
        if($post)
        {
            ...
            if($cek_id > 0)
            {
                ...
                if($update)
                {
                    ...
                    role="alert"><strong>Success </strong>Data berhasil Di Update! <i class="remove ti-close" data-dismiss="alert"></i></div>');
                    redirect('admin/Settings');                    
                } else {
                    $this->session->set_flashdata('message', '<div class="alert alert-danger alert-dismissible fade show" 
                    role="alert"><strong>Upsss</strong>Data Gagal di Update <i class="remove ti-close" data-dismiss="alert"></i></div>');
                    redirect('admin/Settings');           
                }
            } else {
                $this->session->set_flashdata('message', '<div class="alert alert-warning alert-dismissible fade show" 
                role="alert"><strong>Upsss</strong>Maaf ID Tidak Ditemukan. <i class="remove ti-close" data-dismiss="alert"></i></div>');
                redirect('admin/Settings'); 
            }
    ...
    private function _do_upload($fileName)
    {
        ...
        if(!$this->upload->do_upload('logo')) // upload dan validasi
        {
